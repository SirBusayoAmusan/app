...
export const PROMPT_CATALOGUE = [
  { key: 'global', title: 'The rules it always follows', version: PROMPT_SET_VERSION, applies: 'Added in front of every single request.', body: GLOBAL_SYSTEM_PROMPT },
  { key: 'query_strategist', title: 'Deciding what to search for', version: PROMPT_SET_VERSION, applies: 'Suggests what to look up. It is never allowed to make a claim about the market.', body: QUERY_STRATEGIST_PROMPT },
  { key: 'signal_extraction', title: 'Reading what people actually said', version: PROMPT_SET_VERSION, applies: 'Turns the pages we read into points, each tied to the page it came from.', body: SIGNAL_EXTRACTION_PROMPT },
  { key: 'niche_differentiation', title: 'Narrowing to real people', version: PROMPT_SET_VERSION, applies: 'Turns those points into specific groups, never broad categories.', body: NICHE_DIFFERENTIATION_PROMPT },
  { key: 'problem_mining', title: 'Finding the specific problems', version: PROMPT_SET_VERSION, applies: 'Names the problems those people actually have.', body: PROBLEM_MINING_PROMPT },
  { key: 'adversarial_validation', title: 'Trying to prove it wrong', version: PROMPT_SET_VERSION, applies: 'Actively hunts for reasons each opportunity would fail, before it is scored.', body: ADVERSARIAL_VALIDATION_PROMPT },
  { key: 'scoring_analyst', title: 'Giving each part a mark', version: PROMPT_SET_VERSION, applies: 'Marks the individual parts with reasons and links. It never sets the final score.', body: SCORING_ANALYST_PROMPT },
  { key: 'opportunity_report', title: 'Explaining the result', version: PROMPT_SET_VERSION, applies: 'Explains the score and the sources honestly.', body: OPPORTUNITY_REPORT_PROMPT },
  { key: 'product_strategist', title: 'Choosing what to build', version: PROMPT_SET_VERSION, applies: 'Picks the smallest product that would genuinely solve the problem.', body: PRODUCT_STRATEGIST_PROMPT },
  { key: 'guide_writer', title: 'Writing your product', version: PROMPT_SET_VERSION, applies: 'Writes each chapter of your deliverable to a publishable standard.', body: GUIDE_WRITER_PROMPT },
  { key: 'pricing_strategist', title: 'Setting your prices', version: PROMPT_SET_VERSION, applies: 'Prices based on the value delivered. It never invents what competitors charge.', body: PRICING_STRATEGIST_PROMPT },
  { key: 'marketing_strategist', title: 'Your marketing plan', version: PROMPT_SET_VERSION, applies: 'Builds each marketing piece you need, one module at a time.', body: MARKETING_STRATEGIST_PROMPT },
  { key: 'ad_strategist', title: 'Your ads', version: PROMPT_SET_VERSION, applies: 'Ad ideas, what to test first, and what each budget would get you.', body: AD_STRATEGIST_PROMPT },
  { key: 'optimization_strategist', title: 'What to do next', version: PROMPT_SET_VERSION, applies: 'Reads your real numbers and picks the single next move.', body: OPTIMIZATION_STRATEGIST_PROMPT },
  { key: 'sales_page_writer', title: 'Your sales page', version: PROMPT_SET_VERSION, applies: 'A complete sales page built only from the facts we verified.', body: SALES_PAGE_WRITER_PROMPT },
  { key: 'launch_planner', title: 'Your launch plan', version: PROMPT_SET_VERSION, applies: 'A realistic day-by-day launch over about two weeks.', body: LAUNCH_PLANNER_PROMPT },
];
