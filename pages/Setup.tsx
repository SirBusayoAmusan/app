...
export default function Setup() {
  ...
  return (
    <Page title={STEPS[stepIndex].title} sub={STEPS[stepIndex].blurb}>
      {/* progress — three dots, one job each */}
      ...
      <div className="max-w-2xl space-y-5">
        ...
        {step === 'ai' ? (
          <Card className="pad">
            <Field label="Paste your AI key">
              ...
              <div className="hint">
                OpenRouter and Groq keys are recognised automatically. {preset.label} costs {preset.cost_note.toLowerCase()}
              </div>
            </Field>
            ...
            <button className="linkbtn no-underline text-[12px] text-ink-mute hover:text-ink mt-5" onClick={() => setShowAdvanced((v) => !v)}>
              {showAdvanced ? 'Hide options' : 'Model and provider options'}
            </button>
        ...
        <p className="text-[12px] text-ink-faint leading-relaxed flex items-start gap-2">
          ...
          <span>
            Keys live in this browser tab's memory only — never written to storage, never sent anywhere except the provider you chose,
